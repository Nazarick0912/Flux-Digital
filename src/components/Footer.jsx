...
const Footer = () => {
  return (
    <div id="contact" className="w-full bg-[#0a192f] text-gray-300 py-16 px-4 font-sans relative">
      ...
      {/* --- THE FADE DIVISOR LINE (Top Separation) --- */}
      ...
      <div className="max-w-[1240px] mx-auto">
        ...
        {/* --- PART 1: GLOWING NEWSLETTER SECTION --- */}
        <div className="flex flex-col items-center text-center mb-16 relative">

            {/* Subtle glow behind the text */}
            <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[600px] h-[200px] bg-[#00df9a] rounded-full blur-[120px] opacity-10 pointer-events-none"></div>

            <h2 className="text-3xl md:text-5xl font-bold text-white mb-6 relative z-10">
              Ready to <span className="text-transparent bg-clip-text bg-gradient-to-r from-[#00df9a] to-blue-500">Scale Up?</span>
            </h2>
            ...
            {/* Modern Input Group */}
            <form className="flex flex-col sm:flex-row items-center gap-4 w-full max-w-xl relative z-10" onSubmit={(e) => e.preventDefault()}>
              <input
                type="email"
                placeholder="Enter your email address"
                className="w-full sm:w-2/3 p-4 bg-[#112240] border border-gray-600 text-white rounded-lg focus:outline-none focus:border-[#00df9a] focus:ring-1 focus:ring-[#00df9a] transition placeholder-gray-500 shadow-lg"
              />
              <button className="w-full sm:w-1/3 py-4 bg-gradient-to-r from-[#00df9a] to-emerald-400 text-black font-bold rounded-lg transition duration-300 hover:scale-105 hover:shadow-[0_0_20px_rgba(0,223,154,0.4)]">
                Subscribe
              </button>
            </form>
        </div>
        ...
        {/* Internal Divider */}
        <div className="w-full h-[1px] bg-gray-800 my-10"></div>

        {/* --- PART 2: BOTTOM LINKS GRID --- */}
        <div className="grid grid-cols-2 md:grid-cols-4 gap-8">
          ...
          <div>
            ...
            <ul className="space-y-3 text-sm text-gray-400">
              {['Custom Software', 'Cloud Architecture', 'IoT Integration', 'Cybersecurity'].map((item) => (
                <li key={item} className="hover:text-[#00df9a] hover:translate-x-2 transition-all duration-300 cursor-pointer">
                  {item}
                </li>
              ))}
            </ul>
          ...
          <div>
            ...
            <ul className="space-y-3 text-sm text-gray-400">
              {['About Us', 'Our Team', 'Careers', 'Press & Media'].map((item) => (
                <li key={item} className="hover:text-blue-400 hover:translate-x-2 transition-all duration-300 cursor-pointer">
                  {item}
                </li>
              ))}
            </ul>
          ...
          {/* Column 3 */}
          <div>
            <h3 className="font-bold text-white text-lg mb-6 pl-2 border-l-4 border-purple-500">Legal</h3>
            <ul className="space-y-3 text-sm text-gray-400">
              {['Terms of Service', 'Privacy Policy', 'Cookie Settings', 'Accessibility'].map((item) => (
                <li key={item} className="hover:text-purple-400 hover:translate-x-2 transition-all duration-300 cursor-pointer">
                  {item}
                </li>
              ))}
            </ul>

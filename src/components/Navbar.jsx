  ...
  return (
    <div className='absolute top-0 left-0 w-full z-10'>
        <div className='container mx-auto flex justify-between items-center py-4 px-6 md:px-20 lg:px-32 bg-transparent'>
            <img src={assets.logo} alt="Flux Digital" className="w-40 sm:w-52"/>
            <ul className='hidden md:flex gap-7 text-white'>
                <a href="#Header" className='cursor-pointer hover:text-[#00df9a] transition-colors'>Home</a>
            ...
            <button className='hidden md:block bg-[#00df9a] text-black font-medium px-6 py-2 rounded-full cursor-pointer hover:bg-[#00c489] transition-colors'>Sign Up</button>
            <img onClick={() => setShowMobileMenu(true)} src={assets.menu_icon} className='md:hidden w-6 cursor-pointer' alt="Menu" />
        </div>
        ...
        <div className={`md:hidden ${showMobileMenu ? 'fixed w-full' : 'h-0 w-0'} right-0 top-0 bottom-0 overflow-hidden bg-[#0a192f]/95 backdrop-blur-md transition-all`}>
          ...
          <ul className='flex flex-col items-center gap-2 mt-5 px-5 text-lg font-medium text-white'>
              <a onClick={() => setShowMobileMenu(false)} href="#Header" className='px-4 py-2 rounded-full inline-block hover:text-[#00df9a] transition-colors'>Home</a>
